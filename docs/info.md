...
3. Run STA analysis to see if there are any timing violations.
4. On-Hardware Validation
...
The primary goal is to verify the functional and timing correctness of the Whack-a-Mole ASIC design. Our strategy relies on a comprehensive, Python-based cocotb testbench to simulate the design's behavior in response to various stimuli. Verification will be performed at three stages: pre-layout (RTL) and post-layout (gate-level), and STA analysis. We have created 14 testcases to test all the functionality of our game logic.
